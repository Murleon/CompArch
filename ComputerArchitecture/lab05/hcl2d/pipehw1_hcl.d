...
int tick(bool showpc=true, bool showall=false) {
    ...
    ulong _HCL_e_valE = (((_HCL_E_icode)==(3)) ? (_HCL_E_valC) :
    ...
    ulong _HCL_need_regs = (((_HCL_f_icode)==(2)))||(((_HCL_f_icode)==(3)));
    _HCL_need_regs &= 0x1;
    if (showall) writefln("set need_regs to 0x%x",_HCL_need_regs);
    ulong _HCL_need_immediate = ((_HCL_f_icode)==(3));
    _HCL_need_immediate &= 0x1;
    if (showall) writefln("set need_immediate to 0x%x",_HCL_need_immediate);
    ulong _HCL_f_rA = ((_HCL_need_regs) ? (cast(ulong)(((_HCL_i10bytes)>>12UL)&0xf)) :
		(15));
    _HCL_f_rA &= 0xf;
    if (showall) writefln("set f_rA to 0x%x",_HCL_f_rA);
    ulong _HCL_f_rB = ((_HCL_need_regs) ? (cast(ulong)(((_HCL_i10bytes)>>8UL)&0xf)) :
		(15));
    _HCL_f_rB &= 0xf;
    if (showall) writefln("set f_rB to 0x%x",_HCL_f_rB);
    ulong _HCL_f_valC = (((_HCL_need_immediate)&&(_HCL_need_regs)) ? (cast(ulong)(((_HCL_i10bytes)>>16UL)&0xffffffffffffffff)) :
		(_HCL_need_immediate) ? (cast(ulong)(((_HCL_i10bytes)>>8UL)&0xffffffffffffffff)) :
		(0UL));
    _HCL_f_valC &= 0xffffffffffffffff;
    if (showall) writefln("set f_valC to 0x%x",_HCL_f_valC);
    ulong _HCL_f_valP = (((_HCL_need_immediate)&&(_HCL_need_regs)) ? ((_HCL_pc)+(10UL)) :
		(_HCL_need_immediate) ? ((_HCL_pc)+(9UL)) :
		(_HCL_need_regs) ? ((_HCL_pc)+(2UL)) :
		((_HCL_pc)+(1UL)));
    _HCL_f_valP &= 0xffffffffffffffff;
    if (showall) writefln("set f_valP to 0x%x",_HCL_f_valP);
    ulong _HCL_p_pc = ((_HCL_f_valP));
    _HCL_p_pc &= 0xffffffffffffffff;
    if (showall) writefln("set p_pc to 0x%x",_HCL_p_pc);
    _HCL_stall_P = cast(bool)((_HCL_d_Stat)!=(1));
    if (showall) writefln("set stall_P to %s",_HCL_stall_P);
    ulong _HCL_f_Stat = (((_HCL_f_icode)==(0)) ? (2) :
		(((((_HCL_f_icode)==(1)))||(((_HCL_f_icode)==(2))))||(((_HCL_f_icode)==(3)))) ? (1) :
		(4));
    _HCL_f_Stat &= 0x7;
    if (showall) writefln("set f_Stat to 0x%x",_HCL_f_Stat);
    ulong _HCL_d_valA = (((_HCL_reg_srcA)==(_HCL_e_dstE)) ? (_HCL_e_valE) :
...
int main(string[] args) {
	...
	write(`| register dE(`,(_HCL_bubble_E?'B':_HCL_stall_E?'S':'N'));
		...
		writefln(`|  valA=%016x valC=%016x }                        |`, _HCL_E_valA, _HCL_E_valC);
